...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    async_add_entities(
        [
            ...
            RecommendedPowerSensor(coordinator),
...
class RecommendedPowerSensor(_BaseSensor):
    _attr_native_unit_of_measurement = UnitOfPower.WATT

    def __init__(self, coordinator) -> None:
        super().__init__(coordinator, "recommended_power", "Recommended heating power")

    @property
    def native_value(self):
        value = self.coordinator.data.get(ATTR_RECOMMENDED_POWER_NOW)
